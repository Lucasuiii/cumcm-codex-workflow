...
所有 SHA-256、`sha256-tree-v1` 源码树、PDF 页数与哈希都由 `record_run.py` / `record_compile.py` / `refresh_evidence.py` 观察写入，不由 agent 手填；`index_result.py` 通过 locator 读回数值，因此索引值与执行输出在结构上不可能不一致。
...
Independent review package 的 package/upstream digest 只覆盖正式结果引用的 canonical evidence 与复核说明；失败/探索 run、stdout/stderr、旧实验和 debug history 默认不进入包，也不扩大 freshness 范围。包内 `context_excluded` 明确列出打包器实际排除了哪些先验推理——这是打包器能诚实断言的事实，而不是关于 reviewer 心理状态的声明。
